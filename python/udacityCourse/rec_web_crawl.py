...
def rec_crawl(seed, crawled, count, max_depth):
    if(count >= max_depth): return
    crawled.append(seed)
    for link in get_all_links(get_page(seed)):
        if(link in crawled): continue
        else: rec_crawl(link,crawled,count+1,max_depth)
            
def crawl_web(seed,max_depth):
    crawled = []
    count = -1
    rec_crawl(seed,crawled,count,max_depth)
    return crawled
